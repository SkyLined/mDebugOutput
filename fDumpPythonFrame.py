...
def fDumpPythonFrame(oPythonFrame, sPrefix = "", sSuffix = "", bExpand = True):
  ...
  try:
    ...
    while oPythonFrame:
      uIndex += 1;
      print("%sF#%d%s %s @ %s%s%d%s" % (
        sPrefix, uIndex, "*" if "mDebugOutput_HideInCallStack" in oPythonFrame.f_code.co_varnames else " ",
        oPythonFrame.f_code.co_name,
        oPythonFrame.f_code.co_filename,
        dxConfig["sLineNumberAfterPathPrefix"],
        oPythonFrame.f_lineno,
